...
def _extract_public_location(*items: Dict) -> str:
    """Return a public coarse location field if the platform response exposes one."""
    for item in items:
        if not isinstance(item, dict):
            continue
        for key in _PUBLIC_LOCATION_KEYS:
            value = item.get(key)
            if value:
                return str(value)
    return ""
...
async def update_xhs_note(note_item: Dict):
    ...
    local_db_item = {
        ...
        "public_ip_location": _extract_public_location(note_item, user_info),
    }
...
async def update_xhs_note_comment(note_id: str, comment_item: Dict):
    ...
    local_db_item = {
        ...
        "public_ip_location": _extract_public_location(comment_item, user_info),
    }
